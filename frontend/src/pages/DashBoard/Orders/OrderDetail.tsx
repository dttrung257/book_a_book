...
const OrderDetail = () => {
    ...
    useEffect(() => {
        const getInfo = async () => {
            try {
                ...
                const responseOrderDetails = await axios.get(`/orders/${id}/orderdetails`,
                    {
                        headers: {
                            Authorization: `Bearer ${accessToken}`,
                          },
                ...
                if (responseOrder.data.userId) {
                    const responseUser = await axios.get(`manage/users/${responseOrder.data.userId}`, {
                        headers: {
                        Authorization: `Bearer ${accessToken}`,
                        },
                    });
    ...
    const changeOrderStatus = async (status: string) => {
        try {
            const res = await axios.put(`manage/orders/${id}`, 
              {
                status: status
              },
              {
    ...
    const deleteOrder = async () => {
        if (orderInfo && (orderInfo.status === 'SUCCESS' || orderInfo.status === "SHIPPING")) {
            setMessage({
                status: "fail",
    ...
    return (
        <>  
            <div className={`${style.header} mb-2`}>
                ...
                <div>
                    <Button
                        style={{ backgroundColor: "var(--primary-color)", color: "white" , marginRight: "10px"}}
                        onClick = {() => {
                            setStatusModal(true);
                            setMessage(null);
                        }}
            ...
            <div className={`${style.content} d-flex flex-column`}  style={{minHeight: "85vh"}}>
                <SubContainer>
                    <h5>Order ID #{orderInfo?.id}</h5>
                    <div className="d-flex mx-3">
                        <div className="flex-fill">
                            <p><span className="fw-bold">Status: </span>{orderInfo?.status}</p>
                            <p><span className="fw-bold">Order Date: </span>{orderInfo?.orderDate}</p>
                        </div>
                        <div className="flex-fill">
                        <p><span className="fw-bold">Total: </span>${orderInfo?.total}</p>
                            <p><span className="fw-bold">Quantity: </span>{orderInfo?.quantity}</p>
                        </div>
                    </div>
                </SubContainer>
                {orderInfo?.userId && 
                    <SubContainer className="flex-row pb-1" style={{minHeight: '125px'}}>
                        <div className="flex-fill ps-2" style={{borderRight: "solid 2px #cbcbcb", maxWidth: '50%'}}>
                            <h5>Customer</h5>
                            <div className="d-flex">
                                ...
                                />
                                <div className="ms-3">
                                    <span className="fw-bold">{userInfo?.lastName} {userInfo?.firstName}</span><br/>
                    ...
                    <Table >
                        ...
                        <tbody>
                            {orderDetails.map((order, index) => (
                                <tr key = {order.id}>
                                    <td>{index + 1}</td>
                                    <td><img src={order.image} alt="bookimg" style={{height: "60px"}}/></td>
                            ...
                            <tr className="fs-5 lh-lg">
                                <td colSpan={5} className="fw-bold text-start">Total</td>
