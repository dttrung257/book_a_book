...
interface ModalProps {
	onHide: () => void, show: boolean, bookid: number,
	sent: boolean
...
const ModalComment = (props: ModalProps) => {
  ...
  const [content, setContent] = useState<string>(props.comment !== undefined ? props.comment.content : "")
	const [value, setValue] = useState<number | null>(3);
  const [hover, setHover] = useState(-1);
	const [rate, setRate] = useState<number>(props.comment !== undefined ? props.comment.rate : 0);
	...
	const sendReview = () => {
		if (props.comment !== undefined) {
			if (props.comment.content === content && props.comment.rate === rate) props.onHide();
			updateComment({bookId: props.bookid, star: rate, content: content}, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      })
		} else addComment({bookId: props.bookid, star: rate, content: content}, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      })
  ...
  return (
    ...
    >
      ...
      <Modal.Body>
        ...
        />
        <Form.Control style={{marginTop: "10px"}} as="textarea" rows={5} placeholder="Write a comment..." value={content} onChange={handleChangeContent}/>
        <p style={{color: "#666", fontSize: "14px"}}>Character count: {content === undefined ? 0 : content.length}/1000</p>
        <hr />
    ...
    >
      ...
      <Modal.Footer>
        ...
        &nbsp;&nbsp;
        <Button variant="contained" color="primary" size="small" onClick={sendReview}>
          Send
        </Button>
      </Modal.Footer>
    </Modal>
