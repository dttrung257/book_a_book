...
const Comment = (props: { id: number, rate: number | undefined }) => {
  ...
  return (
    ...
    >
      ...
      {isLoggedIn ? (comment === "" ? (<Fragment><ModalComment
        show={toggle}
        onHide={changeToggle}
        bookid={props.id}
      ...
      <Button variant="contained" color="primary" style={{ float: "right" }} size="small" onClick={changeToggle}>Comment</Button>
      </div>
      </Fragment>
      ) : (
        <Fragment>
      <div className="frame-comment">
        ...
        <div className="btnFrame" style={{marginTop: "10px"}}>
          <Button variant="contained" color="error" style={{ float: "right" }} size="small" onClick={() => setToggleDel(true)}> <MdDelete />&nbsp;Delete</Button>
          <Button variant="contained" color="primary" style={{ float: "right", marginRight: "10px" }} onClick={changeToggle} size="small"> <MdEdit />&nbsp;Edit</Button>
        </div>
        </Fragment>
      )) : <></>}
      <div>
      <div className="frame-comment">
        ...
        <div className="textField">
          <p className="ownerCmt">Admin </p><Star rate={5} />
          <p>Because, despite the fairly innocuous first 200 pages, the title speaks the truth: this is a book about war. All of its horrors and atrocities. It is not sugar-coated, and it is often graphic. The "poppy" aspect refers to opium, which is a big part of this book. It is a fantasy, but the book draws inspiration from the Second Sino-Japanese War and the Rape of Nanking.</p>
        </div>
      </div><br/></div>
      {otherComments.length > 0 && otherComments.map((cmt, i) => {
        return (
          <div key={i}>
      <div className="frame-comment">
        <div className="frame-avatar">
          <Avatar src={cmt.avatar} style={{ maxWidth: 30, maxHeight: 30}} />
        </div>
        <div className="textField">
          <p className="ownerCmt">{cmt.fullName} </p><Star rate={cmt.star} />
          <p>{cmt.content}</p>
        </div>
      </div><br/></div>
          )
      ...
      <Modal.Footer>
        ...
        &nbsp;&nbsp;
        <Button variant="contained" color="primary" size="small" onClick={() => setToggleDel(false)}>
          Cancel
        </Button>
      </Modal.Footer>
      </Modal>
      <div style={{width: "100%"}}>
      <Pagination count={totalPage} page={page + 1} color="primary" style={{margin: "auto"}} onChange={handleChangePage}/>
      </div>
    </div>
  );
};
